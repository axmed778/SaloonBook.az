...
// Split out of ReconcileBar so the dialog's a11y hook (focus trap, Escape,
// focus restore) mounts and unmounts with the dialog itself.
function ReconcileDialog({
  ...
  overdue: CalendarBlock[];
  error: string | null;
  pending: boolean;
  busyId: string | null;
  onResolve: (id: string, status: "COMPLETED" | "NO_SHOW") => void;
  onClose: () => void;
}) {
  const t = useTranslations("Calendar");
              ...
              <li key={b.id} className="rounded-lg border border-border bg-muted/40 p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <button
                    ...
                    onClick={() => onResolve(b.id, "COMPLETED")}
                  ...
                  <button
                    type="button"
                    disabled={pending && busyId === b.id}
                    onClick={() => onResolve(b.id, "NO_SHOW")}
